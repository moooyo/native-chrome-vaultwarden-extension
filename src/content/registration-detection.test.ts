...
import { detectRegistrationFields, matchRegistrationField, findConfirmPasswordFields } from './registration-detection.js';
...
describe('registration-detection', () => {
  ...
  it('finds the confirm password field to co-fill, excluding the generated field and non-password inputs', () => {
    const f = form(`
      <input type="text" name="username" />
      <input type="password" name="new-password" autocomplete="new-password" />
      <input type="password" name="confirm-password" autocomplete="new-password" />
    `);
    const pw = f.querySelector('[name="new-password"]') as HTMLInputElement;
    const confirms = findConfirmPasswordFields(pw);
    expect(confirms).toEqual([f.querySelector('[name="confirm-password"]')]);
  });

  it('returns no confirm fields when the form has none', () => {
    const f = form(`<input type="password" name="new-password" autocomplete="new-password" />`);
    expect(findConfirmPasswordFields(f.querySelector('input')!)).toEqual([]);
  });
});
