...
describe('autofill controller', () => {
  ...
  it('opens the card side panel only on focus, anchored to the focused field', () => {
    vi.mocked(sendRequest).mockResolvedValue({ ok: true, data: [] });
    document.body.innerHTML = '<form><input autocomplete="cc-number" id="num"><input autocomplete="cc-csc" id="csc"></form>';

    startAutofill('https://shop.example/checkout');
    const cardPopover = popoverState.instances.at(-1)!;
    // Card/identity now use the side panel (design 2c), like login — no always-visible trigger.
    expect(cardPopover.options.sidePanel).toBe(true);
    // Nothing is shown until a card field is focused.
    expect(cardPopover.open).not.toHaveBeenCalled();

    const csc = document.getElementById('csc') as HTMLInputElement;
    csc.dispatchEvent(new FocusEvent('focusin', { bubbles: true }));

    // Opened, re-anchored to the focused field (not the form anchor).
    expect(cardPopover.open).toHaveBeenCalledWith(csc);
  });
  ...
  it('keeps only one fill panel open when a form has both card and identity fields', () => {
    vi.mocked(sendRequest).mockResolvedValue({ ok: true, data: [] });
    // One form carrying BOTH card and identity fields → a card popover and an identity popover attach.
    document.body.innerHTML = '<form><input autocomplete="cc-number" id="num"><input autocomplete="given-name" id="fn"><input autocomplete="family-name" id="ln"><input autocomplete="street-address" id="st"><input autocomplete="postal-code" id="zip"></form>';
    startAutofill('https://shop.example/checkout');
    const card = popoverState.instances.find((p) => p.options.kind === 'card')!;
    const identity = popoverState.instances.find((p) => p.options.kind === 'identity')!;

    // Focus a card field → the card panel opens.
    (document.getElementById('num') as HTMLInputElement).dispatchEvent(new FocusEvent('focusin', { bubbles: true }));
    expect(card.open).toHaveBeenCalled();

    // Focus an identity field in the SAME form → the identity panel opens and the card panel is dismissed.
    (document.getElementById('fn') as HTMLInputElement).dispatchEvent(new FocusEvent('focusin', { bubbles: true }));
    expect(identity.open).toHaveBeenCalled();
