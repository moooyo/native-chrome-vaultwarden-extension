...
// ORG_KEY_VECTOR: a fixed 64-byte organization symmetric key (orgKeyHex) wrapped under the
// RSA_VECTOR public key as an encType=4 RSA-OAEP-SHA1 EncString (encOrgKey). This reproduces the
// organization-key unwrap chain: Profile.organizations[].key -> rsaOaepDecrypt(RSA_VECTOR private
// key) -> 64-byte org SymmetricKey. RSA-OAEP is randomized, so encOrgKey was captured once from
// tools/gen-vectors.mjs; it always decrypts back to orgKeyHex with RSA_VECTOR.privateKeyPkcs8B64.
export const ORG_KEY_VECTOR = {
  orgKeyHex:
